...
async function main() {
	...
	let emailID = args[1].toString();
	let ID = 0;
        const walletPath = path.join(process.cwd(), `identity/user/${hospitalID}-user/wallet`);
    ...
    try {
        ...
        if ( hospitalID.toString() == 'hospital1' )
        	ID = 500;
        
        if ( hospitalID.toString() == 'hospital2' )
        	ID = 600;
        	
        	
        const userName = `${hospitalID}-user`;
        ...
        const issueResponse = await contract.submitTransaction('registerHospitalAdmin', JSON.stringify(adminDetails));
